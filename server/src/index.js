...
console.log(
  "☁️ Cloudinary configured with cloud name:",
...
const limiter = rateLimit({
  ...
  handler: (req, res) => {
    console.log(
      `Rate limit reached for IP: ${req.ip}, path: ${req.path}, method: ${req.method}`
    );
    res
      .status(429)
      .json({
  ...
  cors({
    origin: function (origin, callback) {
      ...
      if (
        ...
        origin.includes("rymaacademy.cloud")
      ) {
        return callback(null, true);
      }
...
app.use((req, res, next) => {
  ...
  if (req.method === "OPTIONS") {
    ...
    res.header(
      ...
      process.env.NODE_ENV === "production" ? "86400" : "1"
    );
...
app.use(
  ...
  express.static(path.join(__dirname, "../src/uploads"), {
    ...
    setHeaders: (res, filePath) => {
      ...
      // Cache videos more aggressively
      if (
...
app.use((err, req, res, next) => {
  ...
  if (!res.headersSent && res.writable) {
    ...
    console.log(
      "Response already sent or connection closed - skipping error response"
    );
  }
});
