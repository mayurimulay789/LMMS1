...
class CertificateService {
  constructor() {
    // No local directory needed - we use Cloudinary for certificates
  }
  ...
  async generateCertificate(data) {
    try {
      ...
      // Wait for PDF to be generated in memory
      ...
      // // Upload PDF to Cloudinary (certificates only)
      ...
      // In the generateCertificate method, update the upload call:
      const cloudinaryUrl = await uploadToCloudinary(
        pdfBuffer,
        'lms/certificates',
        {
          resource_type: 'raw', // Explicitly set to raw for PDF
          filename: `${certificateId}.pdf`, // This helps detect it's a PDF
          use_filename: true
        }
      );
      ...
      try {
        await certificate.save()
        console.log(`✅ Certificate saved with Cloudinary URL: ${certificateId}`)
      } catch (saveError) {
        console.error("Certificate DB save error:", saveError)
