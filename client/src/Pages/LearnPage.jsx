...
const LearnPage = () => {
  ...
  const renderVideoPlayer = (lesson) => {
    if (!lesson?.videoUrl) return (
      <div className="flex flex-col items-center justify-center h-48 p-4 bg-gray-100 rounded-lg sm:h-64 md:h-80">
    ...
    if (videoId && isYouTubeLoaded) {
      return (
        <div className="relative w-full overflow-hidden rounded-lg" style={{ height: 'clamp(300px, 50vw, 500px)' }}>
          <div
    ...
    return (
      <div className="relative w-full overflow-hidden rounded-lg" style={{ height: 'clamp(300px, 50vw, 700px)' }}>
        <video
          src={lesson.videoUrl}
          controls
  ...
  const renderModuleSidebar = () => (
    <>
      <div className="p-3 bg-white rounded-lg shadow-sm sm:p-4">
        ...
        <div className="space-y-2 overflow-y-auto max-h-80 sm:max-h-96">
  ...
  const renderMobileSidebarContent = () => (
    <>
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <h2 className="text-base font-semibold sm:text-lg">Course Content</h2>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="grid grid-cols-1 gap-4 px-3 py-3 mx-auto max-w-7xl sm:px-4 sm:py-4 lg:grid-cols-4 sm:gap-5 lg:gap-6">
        {/* Sidebar - Mobile Overlay */}
        {isSidebarOpen && (
          <div className="fixed inset-0 z-40 bg-black bg-opacity-50 lg:hidden" onClick={() => setIsSidebarOpen(false)}>
            <div className="absolute top-0 left-0 w-80 max-w-sm h-full p-4 overflow-y-auto bg-white shadow-lg" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="lg:col-span-3">
          {selectedLesson ? (
            ...
            >
              <div className="mb-3 sm:mb-4">
                <div className="flex items-center text-xs text-gray-600 mb-2 sm:text-sm">
                  ...
                  {course.modules.map(module => {
                    ...
                    if (lessonInModule) {
                      return (
                        <span key={module._id} className="px-2 py-1 bg-primary-100 text-primary-800 rounded text-2xs sm:text-xs">
                          {module.name}
                        </span>
                      )
                    }
              ...
              {isLessonCompleted(selectedLesson._id) && (
                <div className="flex items-center p-3 mt-3 space-x-3 border border-green-200 rounded-lg bg-green-50 sm:p-4 sm:mt-4">
                  <CheckCircle className="w-4 h-4 text-green-600 sm:h-5 sm:w-5" />
                  <div>
                    <p className="text-xs font-medium text-green-800 sm:text-sm">Lesson Completed!</p>
                    <p className="text-2xs text-green-600 sm:text-xs">Great job! Continue to the next lesson.</p>
                  </div>
                </div>
              )}
                  ...
                  <div className="space-y-2">
                    {selectedLesson.resources.map((res, idx) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium text-gray-900 truncate sm:text-sm">{res.title}</p>
                          <p className="text-2xs text-gray-600 sm:text-xs">Click to download/view</p>
                        </div>
                      </a>
                    ))}
                  </div>
            ...
            <div className="p-6 text-center bg-white rounded-lg shadow-sm sm:p-8 md:p-12">
              <Play className="w-10 h-10 mx-auto mb-2 text-gray-300 sm:h-12 sm:w-12 sm:mb-3 md:h-16 md:w-16" />
              <h3 className="mb-1 text-base font-semibold text-gray-900 sm:text-lg md:text-xl">Select a Lesson</h3>
              <p className="text-xs text-gray-600 sm:text-sm md:text-base">Choose a lesson from the sidebar to start learning</p>
              <button
                onClick={() => setIsSidebarOpen(true)}
                className="px-4 py-2 mt-3 text-xs text-white transition-colors bg-blue-600 rounded-lg sm:hidden hover:bg-blue-700"
              >
                Open Lessons
              </button>
            </div>
