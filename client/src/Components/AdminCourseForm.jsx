...
const AdminCourseForm = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (modalRef.current && !modalRef.current.contains(event.target)) {
        // Uncomment the line below if you want to close modal when clicking outside
        // resetForm();
      }
    };
  ...
  const addModule = () => {
    ...
    setFormData(prev => ({
      ...
      modules: [
        ...
        {
          id: newModuleId,
          _id: newModuleId, // Add _id for compatibility
  ...
  const updateModuleName = (moduleId, name) => {
    setFormData(prev => ({
      ...
      modules: prev.modules.map(module =>
        module.id === moduleId || module._id === moduleId 
          ? { ...module, name } 
          : module
      )
    }))
    ...
    if (formData.modules.length > 1) {
      setFormData(prev => ({
        ...
        modules: prev.modules
          .filter(module => module.id !== moduleId && module._id !== moduleId)
          .map((module, index) => ({ 
            ...module, 
            order: index + 1 
          }))
      ...
      alert("You must have at least one module in the course.")
    }
  }
        ...
        if (module.id === moduleId || module._id === moduleId) {
          ...
          return {
            ...
            subcourses: [
              ...
              {
                id: newLessonId,
                _id: newLessonId, // Add _id for compatibility
                title: "",
                description: "",
                videoUrl: "",
  ...
  const removeLesson = (moduleId, lessonId) => {
    setFormData(prev => ({
      ...
      modules: prev.modules.map((module) => {
        if (module.id === moduleId || module._id === moduleId) {
          const updatedSubcourses = module.subcourses.filter(
            lesson => lesson.id !== lessonId && lesson._id !== lessonId
          )
          return {
            ...module,
            subcourses: updatedSubcourses.map((sub, index) => ({ 
              ...sub, 
              order: index + 1 
            })),
          }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare modules for submission (remove temporary IDs and use _id)
      const modulesForSubmission = formData.modules.map((module, moduleIndex) => ({
        name: module.name,
        order: module.order,
        subcourses: module.subcourses.map((subcourse, lessonIndex) => ({
          title: subcourse.title,
          description: subcourse.description,
          videoUrl: subcourse.videoUrl,
          order: subcourse.order,
          materials: subcourse.materials || [],
          ...(subcourse._id && { _id: subcourse._id }) // Include _id if it exists
        })),
        ...(module._id && { _id: module._id }) // Include _id if it exists
      }))
  ...
  const startEditing = (course) => {
    ...
    // Prepare modules with proper IDs
    const modulesWithIds = (course.modules || []).map((module, index) => ({
      id: module._id || generateId(), // Use existing _id or generate new
      _id: module._id || null, // Keep original _id
      name: module.name || `Module ${index + 1}: New Module`,
      order: module.order || index + 1,
      subcourses: (module.subcourses || []).map((lesson, lessonIndex) => ({
        id: lesson._id || generateId(), // Use existing _id or generate new
        _id: lesson._id || null, // Keep original _id
        title: lesson.title || "",
    ...
    const moduleIds = modulesWithIds.map(module => module.id)
    ...
    if (window.confirm("Are you sure you want to delete this course? This action cannot be undone.")) {
      try {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json()
          alert(`Error: ${errorData.message || 'Failed to delete course'}`)
        }
      } catch (error) {
  ...
  const openCreateCourseModal = () => {
    setIsCreating(true)
    // Add initial module when creating new course
    if (formData.modules.length === 0) {
      ...
      setExpandedModules([newModuleId])
    }
  }
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col gap-4 p-4 bg-white rounded-lg shadow-sm md:flex-row md:items-center md:justify-between md:p-6">
        ...
        <div className={`flex flex-col gap-2 md:flex-row md:space-x-2 ${mobileMenuOpen ? 'block' : 'hidden md:flex'}`}>
          <button
            ...
            className="flex items-center justify-center w-full gap-2 px-4 py-2 text-white transition-colors bg-primary-700 rounded-lg hover:bg-primary-700 md:w-auto"
          >
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6">
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full px-3 py-2 text-sm border border-black rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent md:text-base"
                    />
                  </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    required
                    rows={3}
                    className="w-full px-3 py-2 text-sm border border-black rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent md:text-base md:rows-4"
                  />
                </div>
                ...
                <div>
                        ...
                        <label
                          ...
                          className={`border-2 border-dashed rounded-lg p-3 md:p-4 text-center cursor-pointer flex flex-col items-center justify-center gap-2 md:flex-row md:min-w-[200px] ${
                            isUploading 
                              ...
                              : 'bg-gray-50 border-black hover:bg-gray-100 text-gray-600'
                          }`}
                        >
                  ...
                  {/* Link Mode - Commented out as in original */}
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-4 md:space-y-6">
                    {formData.modules.map((module) => (
                      <div key={module.id} className="border border-black-200 rounded-lg">
                        <div 
                        ...
                        {expandedModules.includes(module.id) && (
                          <div className="p-4 border-t border-black-100 bg-gray-50">
                            <div className="space-y-4">
                              {module.subcourses.map((lesson, lessonIndex) => (
                                <div key={lesson.id} className="p-4 bg-white border border-black-200 rounded-lg">
                                  <div className="flex items-center justify-between mb-4">
                                  ...
                                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                    <div>
                                      ...
                                      <input
                                        ...
                                        className="w-full px-3 py-2 text-sm border border-black rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                        required
                                      />
                                    </div>
                                    ...
                                    <div>
                                      ...
                                      <input
                                        ...
                                        required
                                        className="w-full px-3 py-2 text-sm border border-black rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                      />
                                    </div>
                                  </div>
                              ...
                              {module.subcourses.length === 0 && (
                                <div className="p-4 text-center border-2 border-dashed border-black rounded-lg">
                    ...
                    {formData.modules.length === 0 && (
                      <div className="p-8 text-center border-2 border-dashed border-black rounded-lg">
                ...
                {/* Submit Buttons */}
                <div className="sticky bottom-0 flex flex-col-reverse gap-3 pt-4 mt-4 border-t border-black-200 bg-white md:flex-row md:justify-end md:space-x-4 md:gap-0">
                  <button
                    type="button"
                    onClick={resetForm}
                    className="w-full px-4 py-2 text-gray-700 border border-black rounded-lg hover:bg-gray-50 md:w-auto"
                    disabled={isUploading}
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex items-center justify-center w-full gap-2 px-4 py-2 text-white transition-colors bg-primary-600 rounded-lg hover:bg-primary-700 disabled:bg-primary-400 disabled:cursor-not-allowed md:w-auto"
                    disabled={isUploading}
                  >
      ...
      <div className="bg-white rounded-lg shadow-sm">
        <div className="p-4 border-b border-black-200 md:p-6">
            ...
            courses.map((course) => (
              <div key={course._id} className="p-4 hover:bg-gray-50 md:p-6">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-black-200 md:pt-0 md:border-t-0 md:justify-end md:space-x-2">
                    <div className="flex items-center space-x-2">
                      <button
